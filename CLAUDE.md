...
> **Older releases** (v3.0 through v4.60.1 — 126 rows) live in [CHANGELOG.md](./CHANGELOG.md). The last 11 releases stay inline here for quick reference. Reorg established during the v4.42.3 maintenance pass; re-trimmed in the v4.52.0, v4.54.1, v4.54.13, v4.56.0-morning, v4.59.6 Road-to-200, v4.62.3 TB-v2-roadmap, v4.62.5 Thursday-tech-debt, v4.63.0 3D-View Phase 1, v4.64.0 3D-View Phase 2, v4.65.0 3D-View Phase 3, v4.65.1 OSI UX patch, and v4.66.0 3D-View UI polish consolidations when inline grew past 11.

| Version | Features Added |
|---|---|
| v4.66.0 | **3D View UI polish pass — mockup parity port** (issue [#199](https://github.com/oremosu98/networkplus-quiz/issues/199) polish). **Fifth Friday ship.** User compared the live v4.65.x 3D view to the approved concept mockup (`mockups/tb-3d-view-concept.html`) and asked to bring its richer chrome into production. Four ported elements: **(1) Left vertical camera rail** (52px) replaces the top-chrome Reset Camera + Top buttons with 5 icon buttons — orbit / top-down / zoom-in / zoom-out / fit-all — plus a monospace vertical "CAMERA" label. Zoom scales distance along the look vector (0.85× in, 1.18× out); fit-all computes `Box3().expandByObject` over all device groups with 1.6× padding. **(2) Zones legend** top-right — translucent floating box listing active zones (Internet / DMZ / each VLAN with color swatch + name). New `tb3d.js` export `getActiveZones()` scans tbState once; app.js renders the list. Hidden when no zones exist. **(3) Pinned inspector dock** right-side (340px) — while 3D is active, the existing v4.60.0 `#tb-inspector-pop` gets re-parented into `#tb-3d-inspector-dock`; CSS strips its floating-popup positioning (`position: static`, no shadow, no drag header, no close button, empty-state card shown by default). On Back-to-2D, `tbClose3DView` restores the popup to its original parent via stashed `dataset._tb3dOrigParentId`. 2D's floating-popup flow unchanged. **(4) Initial camera retuned** from (24, 22, 32) to (36, 28, 44) — all devices + zone plates fit on first open. **Layout architecture**: new `.tb-3d-inner` 3-column grid (`52px 1fr 340px`) slots into the 3D host as `flex: 1`. Responsive gate @ <1024px hides the inspector dock (rail + canvas remain). **UAT**: **28 new v4.66.0 assertions** — 8 HTML, 8 CSS, 5 tb3d exports, 5 app.js wiring, 2 regression guards (rail stays first-class, tb3d.js never touches #tb-inspector-pop). Total UAT: 4009 → **4037** passing. Playwright: 79 → **81** passing (+2: rail + legend + dock all visible in 3D, Back-to-2D restores floating popup). Cache: `netplus-v4.66.0`. ~430 LOC across `index.html` + `styles.css` + `tb3d.js` + `app.js` + tests. **63rd ship**. Closes the mockup-parity arc. Phase 4 (scenario 3D tours) still queued. |
