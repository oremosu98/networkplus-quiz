...
```mermaid
flowchart TB
    subgraph OUT[" OUT — beyond the fence "]
        O1["YouTube long-form"]
        O2["Email newsletter"]
        O3["Paid ads from day 1"]
        O4["Creator partnerships"]
    end

    subgraph FENCE[" THE FENCE "]
        subgraph MAYBE["Maybe-later (v2+)"]
            M1["LinkedIn<br/>after 1k paying users"]
            M2["Twitter/X<br/>after 6mo TikTok focus"]
            M3["Product Hunt<br/>after 100+ paying users"]
        end
        subgraph NICE["Nice-to-have"]
            N1["Founder personal brand"]
            subgraph CORE["CORE"]
                C1["TikTok + IG Reels"]
                C2["Reddit engagement"]
                C3["Discord server"]
                C4["Daily build-in-public"]
                C5["Weekly feature post"]
                C6["User success stories"]
                C7["Free lead magnets"]
                C8["No-clickbait principle"]
            end
        end
    end

    classDef core fill:#1a472a,stroke:#0a2e15,color:#fff,stroke-width:2px
    classDef nice fill:#4a7c5f,stroke:#1a472a,color:#fff
    classDef maybe fill:#9bb8a4,stroke:#4a7c5f,color:#000
    classDef out fill:#f0f0f0,stroke:#999,color:#666,stroke-dasharray: 5 5

    class C1,C2,C3,C4,C5,C6,C7,C8 core
    class N1 nice
    class M1,M2,M3 maybe
